...
int smartctl_main(int argc, char **argv);


/**************KongBin 20210118 AddCode********************/
#define TMPBUF_SIZE 5120
extern char tmpBuf[TMPBUF_SIZE];
extern std::vector<std::string> smartRet;
#include <QDebug>
inline void pushVec(const char *fmt,_G_va_list __arg)
{
    memset(tmpBuf,0,TMPBUF_SIZE);
    //    vprintf(fmt,__arg);
    vsprintf(tmpBuf,fmt,__arg);
    smartRet.push_back(std::string(tmpBuf));
}
/***********************************************************/
#endif
